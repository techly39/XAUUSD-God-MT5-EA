...
input int    Breakout_CloseBeyond_Points= 0;     // RELAXED: Was 10, now 3 (0.09 XAUUSD units)
...
input int    Range_Min_Width_Points= 50;   // RELAXED: Was 100, now 50 (1.5 XAUUSD units)
input int    Range_Max_Width_Points= 800;  // Maximum range width
input int    Sweep_Buffer_Points   = 10;   // RELAXED: Was 25, now 10 (0.30 XAUUSD units)
input int    Sweep_Reentry_Confirm_Bars = 2; // Bars to confirm reversal
input bool   Sweep_Use_RSI_Confirm = false; // DISABLED: Was true, now false for testing
...
input double Fixed_Lot                  = 0.01; // Used only if Risk_Mode=0
...
input int    Max_Spread_Points  = 1500; // Max spread in points
...
input double RR_Target       = 2; // Risk:Reward ratio
